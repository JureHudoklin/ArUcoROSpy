...
class ImageConverter(object):
    ...
    def calculate_transform(self, id_main):
        ...
        obj_pose_stamped.header.frame_id = self.camera_frame_id
        obj_pose_stamped.header.stamp = rospy.Time.now()
        obj_pose_stamped.pose = self.obj_transform
        try:
            camera_to_base = self.tf_buffer.lookup_transform("irb120_base", self.camera_frame_id, obj_pose_stamped.header.stamp, rospy.Duration(1.0))
        except:
            rospy.loginfo(" Aruco Detect: Could not lookup transform --> passing")
            rospy.sleep(1)
            return
        #obj_pose_base_frame = self.tf_listener.transformPose("irb120_base", obj_pose_stamped)
        obj_pose_base_frame = tf2_geometry_msgs.do_transform_pose(obj_pose_stamped, camera_to_base)

        obj_pose_base_frame.pose.position.z = 0.235
        matrix = utils.pose_to_matrix(obj_pose_base_frame.pose)
        x_vec = matrix[0:3, 0]
        y_vec = matrix[0:3, 1]
        x_vec[2] = 0
        y_vec[2] = 0
        ...
        matrix[0:3, 1] = y_vec
        matrix[0:3, 2] = z_vec
        obj_pose_base_frame.pose = utils.matrix_to_pose(matrix)

        trans = np.array([obj_pose_base_frame.pose.position.x, obj_pose_base_frame.pose.position.y, obj_pose_base_frame.pose.position.z])

        object_tf.header.frame_id = "irb120_base"
        object_tf.child_frame_id = self.aruco_object_id
        object_tf.transform.translation = obj_pose_base_frame.pose.position
        object_tf.transform.rotation = obj_pose_base_frame.pose.orientation
